...
Handles the standard PNB account statement PDF format.

Column layout: Date | Particulars | Cheque No | Debit | Credit | Balance
"""
import pdfplumber
...
PNB_KEYWORDS = ["punjab national bank", "pnb", "particulars", "cheque no"]

PNB_COLUMNS = {
    "date": ["date", "txn date", "transaction date"],
    "narration": ["particulars", "narration", "description", "remarks"],
    "ref_no": ["cheque no", "cheque number", "ref no", "reference"],
...
def parse(pdf_path: str) -> list[Transaction]:
    """Parse a Punjab National Bank statement PDF and return a list of Transaction objects."""
    transactions = []
        ...
        for page in pdf.pages:
            ...
            for table in tables:
                ...
                for i, row in enumerate(table):
                    if row and any(
                        cell and any(kw in str(cell).lower() for kw in ["date", "particulars", "debit", "credit"])
                        for cell in row
                    ):
                ...
                col_map = {}
                for field, keywords in PNB_COLUMNS.items():
