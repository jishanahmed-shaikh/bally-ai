...
SBI_KEYWORDS = ["state bank of india", "sbi", "txn date", "debit", "credit", "balance"]

SBI_COLUMNS = {
    "date": ["txn date", "transaction date", "date"],
    "narration": ["description", "narration", "particulars", "remarks"],
    "ref_no": ["ref no", "cheque no", "reference number"],
...
def parse(pdf_path: str) -> list[Transaction]:
    """Parse an SBI bank statement PDF."""
    transactions = []
        ...
        for page in pdf.pages:
            ...
            for table in tables:
                ...
                for i, row in enumerate(table):
                    if row and any(
                        cell and any(kw in str(cell).lower() for kw in ["date", "description", "debit", "credit"])
                        for cell in row
                    ):
                ...
                col_map = {}
                for field, keywords in SBI_COLUMNS.items():
