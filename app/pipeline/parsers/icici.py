...
ICICI_KEYWORDS = ["icici bank", "icici", "transaction remarks", "withdrawal amount (inr)"]

ICICI_COLUMNS = {
    "date": ["transaction date", "value date", "date"],
    "narration": ["transaction remarks", "remarks", "narration", "description"],
    "ref_no": ["cheque number", "cheque no", "reference"],
    "withdrawal": ["withdrawal amount", "withdrawal", "debit amount", "debit"],
    "deposit": ["deposit amount", "deposit", "credit amount", "credit"],
...
def parse(pdf_path: str) -> list[Transaction]:
    """Parse an ICICI Bank statement PDF."""
    transactions = []
        ...
        for page in pdf.pages:
            ...
            for table in tables:
                ...
                for i, row in enumerate(table):
                    if row and any(
                        cell and any(kw in str(cell).lower() for kw in ["date", "remarks", "withdrawal"])
                        for cell in row
                    ):
                ...
                col_map = {}
                for field, keywords in ICICI_COLUMNS.items():
