...
Handles the standard Kotak Bank account statement PDF format.

Column layout: Transaction Date | Value Date | Description | Chq/Ref Number | Dr | Cr | Balance
"""
import pdfplumber
...
KOTAK_KEYWORDS = ["kotak mahindra bank", "kotak bank", "kotak", "811"]

KOTAK_COLUMNS = {
    "date": ["transaction date", "txn date", "date"],
    "narration": ["description", "narration", "particulars", "remarks"],
    "ref_no": ["chq/ref number", "chq/ref no", "ref no", "cheque no"],
    "withdrawal": ["dr", "debit", "withdrawal"],
    "deposit": ["cr", "credit", "deposit"],
    "balance": ["balance"],
}
...
def parse(pdf_path: str) -> list[Transaction]:
    """Parse a Kotak Mahindra Bank statement PDF and return a list of Transaction objects."""
    transactions = []
        ...
        for page in pdf.pages:
            ...
            for table in tables:
                ...
                for i, row in enumerate(table):
                    if row and any(
                        cell and any(kw in str(cell).lower() for kw in ["date", "description", "dr", "cr", "balance"])
                        for cell in row
                    ):
                ...
                col_map = {}
                for field, keywords in KOTAK_COLUMNS.items():
