...
Handles the standard Bank of Baroda account statement PDF format.

Column layout: Date | Narration | Cheque No | Debit | Credit | Balance
"""
import pdfplumber
...
BOB_KEYWORDS = ["bank of baroda", "bob", "baroda"]
...
def parse(pdf_path: str) -> list[Transaction]:
    """Parse a Bank of Baroda statement PDF and return a list of Transaction objects."""
    transactions = []
        ...
        for page in pdf.pages:
            ...
            for table in tables:
                ...
                for i, row in enumerate(table):
                    if row and any(
                        cell and any(kw in str(cell).lower() for kw in ["date", "narration", "debit", "credit"])
                        for cell in row
                    ):
                ...
                col_map = {}
                for field, keywords in BOB_COLUMNS.items():
